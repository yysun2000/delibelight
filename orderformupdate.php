...
else if ($od_settle_case == "계좌이체")
{
    switch($default['de_pg_service']) {
        case 'lg':
            include '/lg/xpay_result.php';
            break;
        case 'inicis':
            include '/inicis/pay_result.php';
            break;
        default:
...
{
    include '/kakaopay/kakaopay_result.php';
...
if($tno) {
    if((int)$order_price !== (int)$pg_price) {
        ...
        switch($od_pg) {
            case 'lg':
                include '/lg/xpay_cancel.php';
                break;
            case 'inicis':
                include '/inicis/inipay_cancel.php';
                break;
            case 'KAKAOPAY':
                $_REQUEST['TID']               = $tno;
                $_REQUEST['Amt']               = $amount;
                ...
                include '/kakaopay/kakaopay_cancel.php';
                break;
            default:
...
include_once('/ordermail1.inc.php'); // 메일
include_once('/ordermail2.inc.php'); // 메일
...
goto_url('orderinquiryview.php?od_id='.$od_id.'&amp;uid='.$uid);
?>
